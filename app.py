...
def calculate_dk_points(row):
    score = (row['PTS']) + (row['FG3M'] * 0.5) + (row['REB'] * 1.25) + (row['AST'] * 1.5) + (row['STL'] * 2) + (
                row['BLK'] * 2) - (row['TOV'] * 0.5)
...
def load_and_process_data():
    """
    Loads game logs from STATIC CSV to avoid API timeouts.
    """
    df = pd.DataFrame()
    try:
        ...
        st.error("❌ 'nba_gamelogs_2026.csv' not found! Please upload the file generated by your script.")
        return pd.DataFrame()
    ...
    # Extract Opponent from MATCHUP (e.g. "LAL @ BOS" -> "BOS", "LAL vs. BOS" -> "BOS")
    ...
    for col in stats:
        ...
        df[f'L10_{col}'] = df.groupby('PLAYER_ID')[col].transform(
...
with st.sidebar:
    ...
    if not schedule_df.empty:
        for _, game in schedule_df.iterrows():
            st.markdown(
                f"<div class='game-card'><b>{game['Matchup']}</b><br><span style='color:#bbb'>{game['Status']}</span></div>",
                unsafe_allow_html=True)
    else:
...
if run_btn:
    if not active_teams:
        ...
        with status1:
            st.metric("Games On Slate", len(active_teams) // 2)
            ...
            with status2:
                with st.spinner("Checking Injuries..."):
                    ...
                    if not injury_df.empty:
                        ...
                        exclude_mask = injury_df['Injury Status'].str.contains('Out|Doubtful|Injured Reserve',
                                                                               case=False, na=False)
                        injured_list_norm = injury_df[exclude_mask]['Player_Norm'].tolist()
                    st.metric("Injured Players", len(injured_list_norm))

            with status3:
                st.metric("Model Status", "XGBoost Active")

            usage_df, defense_df = get_usage_and_defense()

            with st.spinner("Loading Stats & Running Model..."):
                df = load_and_process_data()  # Loads static CSV

                if not df.empty:
                    # --- NEW: CALCULATE H2H HISTORY ---
                    # 1. Group by Player + Opponent
                    # 2. Take Mean of last 5 games vs that opponent
                    h2h_df = df.sort_values('GAME_DATE').groupby(['PLAYER_ID', 'OPPONENT'])['DK_PTS'].apply(
                        lambda x: x.tail(5).mean()).reset_index()
                    h2h_df.rename(columns={'DK_PTS': 'H2H_Avg'}, inplace=True)

                    # Create Map: Team ID -> Abbreviation (e.g. 1610612738 -> BOS)
                    # We need this because Schedule gives IDs, but Game Logs give Abbrevs (BOS)
                    team_id_to_abbrev = df[['TEAM_ID', 'TEAM_ABBREVIATION']].drop_duplicates().set_index('TEAM_ID')[
                        'TEAM_ABBREVIATION'].to_dict()
                    ...
                    # --- MERGE H2H STATS ---
                    # Merge on Player ID and the Opponent Abbreviation they are facing today
                    slate = slate.merge(h2h_df, left_on=['PLAYER_ID', 'OPP_ABBREV'], right_on=['PLAYER_ID', 'OPPONENT'],
                                        how='left')
                    # If they've never played this team in dataset, fill H2H with their L10 Avg
                    ...
                    if not slate.empty:
                        ...
                        # --- WEIGHTED PROJECTION ---
                        # Blend: 85% Recent Form (Model) + 15% Matchup History
                        ...
                        if salary_file is not None:
                            try:
                                ...
                                if 'Name' in salaries.columns:
                                    ...
                                    slate = slate.merge(salaries[['Name', 'Salary']], left_on='Matched_Name',
                                                        right_on='Name', how='left')

                                    slate['Salary'] = pd.to_numeric(slate['Salary'], errors='coerce').fillna(0)
                                    slate = slate[slate['Salary'] > 0]
                                    slate['Value'] = slate.apply(
                                        lambda x: x['Proj_DK_PTS'] / (x['Salary'] / 1000) if x['Salary'] > 0 else 0,
                                        axis=1)
                                else:
                                    slate['Salary'] = 0;
                                    slate['Value'] = 0
                        ...
                        with tab1:
                            cols = ['PLAYER_NAME', 'POSITION', 'TEAM_ABBREVIATION', 'OPP_ABBREV', 'Injury Status',
                                    'Def_Rank', 'H2H_Avg', 'Salary', 'Proj_DK_PTS', 'Value']
                            valid_cols = [c for c in cols if c in slate.columns]
                            st.dataframe(slate[valid_cols].sort_values('Value', ascending=False)
                                         .style.format(
                                {'Salary': '${:.0f}', 'Proj_DK_PTS': '{:.1f}', 'H2H_Avg': '{:.1f}', 'Value': '{:.2f}x',
                                 'Def_Rank': '{:.0f}'})
                    ...
                    st.error("❌ Stats file empty or not found.")